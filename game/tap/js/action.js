...
Action.prototype.run2=function(obj){
	//if(obj.action.list.length<=0)return;
	isChange=true;
	//var _l=obj.action.list.length;
	var _l=this.list.length;
	for(var i=0;i<_l;i++){
		try{
...
Action.prototype.run=function(){
	...
	try{
            ...
            if(typeof(this.list[i][0].obj.isChange)!='unfined')this.list[i][0].obj.isChange=true;
			switch(this.list[i][0].run()){
					case	'loop':
						//如果有定义方法,则执行方法
						if(typeof(this.list[i][0].f)!='undefined')this.list[i][0].f.call(this.list[i][0].obj);
...
ZhSpine.prototype.loadJsonText=function(info){
    ...
    $g().ajax({executor:this,url:_url+'.atlas'
        ,success:function(data){
		    info.atlasText=data;
	    }});
    $g().ajax({executor:this,url:_url+'.json'
        ,success:function(data){
...
ZhSpine.prototype.make=function(){
    ...
    var _spine=new ZhSpine.spine({"name":name,scale:_scale,active:_active}).init(this.infoList[arguments[0]['infoName']]);
    var _x=typeof(arguments[0]['x'])!='undefined'?arguments[0]['x']:-1000;
    var _y=typeof(arguments[0]['y'])!='undefined'?arguments[0]['y']:-1000;
    _spine.move(-1000,-1000);
