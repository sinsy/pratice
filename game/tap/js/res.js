...
Res.prototype.loadList=function(_url){
	var _resThis=this;
	this.loadProgress=0;
	$g().ajax({exe:this,url:_url,success:function(data){
...
Res.prototype.loadAni=function(i){
    ...
    $g().ajax({url:'level/'+this.levelName+'/res/animation/'+this.list[i].name+'.json',success:function(data){
...
Res.prototype.loadConfig=function(i){
    ...
    $g().ajax({executor:this,url:'/center/getInfo/'+this.list[i].gameId,success:function(data){
...
Res.prototype.getResIdByName=function(name,type){
    ...
    if(typeof(this.nullId)=='undefined'){
        this.nullId=this.getResIdByName('null');
    }
	return this.nullId;
}
...
Res.prototype.loadJs=function(i){
	var _resThis=this;
    $g().ajax({url:'level/'+this.levelName+'/'+this.list[i].url,success:function(data){
...
Res.prototype.paintImage=function(data){
    var img=data.img;
    ...
    var _l=imageData.data.length;
    //while(_l){
    //    _l-=4;
    //    if(imageData[_l]==0)continue;
    //    if(Math.abs(imageData[_l-3]-117)<10 && Math.abs(imageData[_l-2]-86)<10 && Math.abs(imageData[_l-1]-71)<10){
    //        imageData[_l-3]=0;
    //        imageData[_l-2]=0;
    //        imageData[_l-1]=0;
    //        imageData[_l]=1;
    ...
    for(var i=0;i<_l;i+=4){
        ...
        if(imageData.data[i]==117 && imageData.data[i+1]==86 && imageData.data[i+2]==71){
            imageData.data[i]=0;
    ...
    this.eList.push(canvas);
    return this.eList.length-1;
}
//缓存资源
Res.Cache=function(){
	this.list={};
...
Res.aniFun.prototype={
	...
	,drawChild:function(n,i,w,ctx,data,canvas){
        ...
        if(typeof(_frames)=='undefined')return;
		ctx.save();	
		ctx.fillStyle="#fff";
		ctx.drawImage(
			this.el
...
Res.aniFun.prototype.makeImage=function(data){
    var canvas=document.createElement('canvas');
